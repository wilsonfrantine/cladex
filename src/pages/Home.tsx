import { Play, Zap, BookOpen, ChevronDown } from 'lucide-react'
      ...
      <div className="relative z-10 flex-1 overflow-y-auto scrollbar-none flex flex-col">
        ...
        <div className="flex-1 flex flex-col w-full max-w-sm mx-auto px-5 gap-3">
          ...
          <button
            ...
            className="group w-full rounded-2xl bg-zinc-900/80 border border-zinc-800 hover:border-blue-700/60 hover:bg-zinc-800/70 backdrop-blur-sm transition-colors duration-150 px-5 py-3.5 text-left flex items-center gap-3"
          >
            <div className="w-8 h-8 rounded-lg bg-blue-900/40 border border-blue-800/50 flex items-center justify-center shrink-0">
              <BookOpen className="w-4 h-4 text-blue-400" />
            </div>
            <div className="flex-1 min-w-0">
              <p className="text-sm font-semibold text-zinc-300 group-hover:text-zinc-100 transition-colors">
                Tutorial
              </p>
              <p className="text-xs text-zinc-600 mt-0.5">
                Mono, para e polifilético — conceitos básicos
              </p>
            </div>
          </button>
          ...
          <div className="flex flex-col gap-2">
            ...
            >
              ...
              <ChevronDown
                className={`w-3.5 h-3.5 text-zinc-600 transition-transform duration-200 ${modulesOpen ? 'rotate-180' : ''}`}
              />
            </button>
              ...
              <div className="flex flex-col gap-2">
                {modules.map((mod) => (
                  <button
                    ...
                    className="group w-full rounded-2xl bg-zinc-900/80 border border-zinc-800 hover:border-zinc-600 hover:bg-zinc-800/70 backdrop-blur-sm transition-colors duration-150 px-4 py-3.5 text-left flex items-center gap-3"
                  >
                    ...
                    <Play className="w-3.5 h-3.5 text-zinc-600 group-hover:text-zinc-400 shrink-0 transition-colors" />
                  </button>
                ))}
              </div>
            )}
